...
import time
LICNECE = """
Copyright (c) 2022 Legoshii レゴシ#3660

Toute personne qui obtient une copie de ce logiciel et des fichiers de documentation associés (le « Logiciel ») est autorisée par les présentes à traiter le Logiciel sans restriction, y compris sans limitation les droits d’utilisation, de copie, de modification, de fusion, de publication, distribuer, sous-licencier et/ou vendre des copies du Logiciel, et permettre aux personnes à qui le Logiciel est fourni de le faire, sous réserve des conditions suivantes :
    ...
    def main(self):  # The main function contains the most important code
        ...
        if os.name == "nt":  # If the system is windows
            ...
            ctypes.windll.kernel32.SetConsoleTitleW(
                "Nitro Generator and Checker - Made by Legoshii レゴシ#3660")  # Change the
        else:  # Or if it is unix
            print(f'\33]0;Nitro Generator and Checker - Made by Legoshii レゴシ#3660\a',
...
╚═╝░░╚═╝╚═╝░░╚══╝╚═╝╚═╝░░░░░╚═╝╚══════╝░░░░░░╚═╝░░░░░╚══════╝╚══════╝
        ...
        self.slowType("Made by: Legoshii レゴシ#3660", .02)
        time.sleep(1)  # Wait a little more
        # Print the first question
        self.slowType(
            "\nEntrez le nombre de codes à générer et à vérifier : ", .02, newLine=False)

        try:
            num = int(input(''))  # Ask the user for the amount of codes
        except ValueError:
            input("L’entrée spécifiée n’était pas un nombre.\nAppuyez sur Entrée pour quitter")
            exit()  # Exit program

        if USE_WEBHOOK:
            # Get the webhook url, if the user does not wish to use a webhook the message will be an empty string
            self.slowType(
                "Si vous souhaitez utiliser un webhook Discord, tapez-le ici ou appuyez sur Entrée pour ignorer : ", .02, newLine=False)
            ...
            if webhook is not None:
                DiscordWebhook(  # Let the user know it has started logging the ids
                        url=url,
                        content=f"```Commencé à vérifier les urls\nJ’enverrai ici tous les codes valides```"
                    ).execute()
        ...
        for s in c:  # Loop over the amount of codes to check
            ...
            if os.name == "nt":  # If the system is windows
                ctypes.windll.kernel32.SetConsoleTitleW(
                    f"Nitro Generator and Checker - {len(valid)} Valid | {invalid} Invalid - Made by Legoshii レゴシ#3660")  # Change the title
                print("")
            else:  # If it is a unix system
                # Change the title
                print(
                    f'\33]0;Nitro Generator and Checker - {len(valid)} Valid | {invalid} Invalid - Made by Legoshii レゴシ#3660\a', end='', flush=True)

        print(f"""
Resultats:
 Valides: {len(valid)}
 Invalides: {invalid}
 Valides Codes: {', '.join(valid)}""")  # Give a report of the results of the check

        # Tell the user the program finished
        input("\nLa fin! Appuyez 5 fois sur Entrée pour fermer le programme.")
